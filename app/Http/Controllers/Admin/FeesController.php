...
class FeesController extends Controller
{
    ...
    public function general_fee_report(Request $request){
        $data['title'] = "Generate General Fee Reports";
        $data['current_year_id'] = Helpers::instance()->getCurrentAccademicYear();
        $data['batches'] = Batch::where('id', '<=', $data['current_year_id'])->orderBydesc('id')->get();

        return view('admin.fee.general_report', $data);
    }


    public function general_fee_report_generate(Request $request){
        try{
            ...
            if($data->count() > 0){
                ...
                $headers = [
                    ...
                    'Content-Disposition' => 'attachment; filename=general_fee_report_y'.$year_id.'.csv'
                ];
